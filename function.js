...
function generateFacilitatorTimetable(facilitatorsData, shiftsData, startDate, endDate, locationsData, previewShiftData, previewFacsData, stateData) {
    // Clear date cache for fresh start
    dateCache.clear();
    
    // Handle both JSON strings and JavaScript objects
    const facilitators = typeof facilitatorsData === 'string' ? JSON.parse(`[${facilitatorsData}]`) : `[${facilitatorsData}]`;
    
	const shifts = typeof shiftsData === 'string' ? JSON.parse(`[${shiftsData}]`) : `[${shiftsData}]`;
    
    // Parse locations data if provided
    let locations = null;
    if (locationsData) {
        try {
            locations = typeof locationsData === 'string' ? JSON.parse(`[${locationsData}]`) : `[${locationsData}]`;
        } catch (e) {
    ...
    if (startDate && endDate) {
        ...
        const startDateStr = startDate.includes('T') ? startDate.split('T')[0] : startDate; // Extract YYYY-MM-DD
        const endDateStr = endDate.includes('T') ? endDate.split('T')[0] : endDate; // Extract YYYY-MM-DD
    ...
    if (previewShiftData && previewFacsData) {
        // Parse preview shift data if it's a JSON string
        let previewShift = null;
        try {
            previewShift = typeof previewShiftData === 'string' ? JSON.parse(previewShiftData) : previewShiftData;
        } catch (e) {
            console.error('Failed to parse previewShift:', e);
        }
        
        if (previewShift && previewShift.startDate) {
            const previewFacs = [...new Set(previewFacsData.split(',').map(email => email.trim()).filter(email => email))];
            
            // Parse preview shift date
            const previewDate = parseDateString(previewShift.startDate);
            if (previewDate && previewFacs.length > 0) {
                ...
                previewFacs.forEach(facEmail => {
                    if (!shiftsByDate[previewDate][facEmail]) {
                        shiftsByDate[previewDate][facEmail] = [];
                    }
                
                const previewShiftObj = {
                    startDateTime: previewShift.startDate,
                    endDateTime: previewShift.endDate,
                    locationID: null, // We'll use locationName directly
                    locationName: previewShift.locationName,
                    shiftStatus: previewShift.status || 'MAYBE', // Use the status from previewShift, default to MAYBE
                    isPreview: true,
                    unavailable: false,
                    allDay: false
                };
                
                    shiftsByDate[previewDate][facEmail].push(previewShiftObj);
                });
            }
        }
    }
    ...
    const sortedFacilitators = [...facilitators].sort((a, b) => {
...
window.function = function (facilitatorsData, shiftsData, startDate, endDate, locations, previewShift, previewFacs, state) {
	...
	// and assign default values for undefined inputs
	...
	// Call the main function with individual parameters directly
	return generateFacilitatorTimetable(
		facilitators, 
		shifts, 
		start, 
		end, 
